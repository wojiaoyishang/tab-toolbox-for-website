...
layui.define(["jquery", "miniMenu", "element", "miniTab", "miniTheme"], function (exports) {
    ...
    var miniAdmin = {
        ...
        listen: function () {
             ...
             * 清理
             */
            $('body').on('click', '[data-process]', function () {
                ...
                layer.open({
                    ...
                    btnAlign: 'c',
                    area: ['450px', '300px'],
                    content: '<div style="padding: 15px;">请选择想要对工具箱进程进行的操作，<br />比如重启工具箱进程、结束工具箱进程。' +
                        '<br /><br />请注意：如果此时有人正在使用工具箱的功能，若执行进程操作后会中断程序的执行。有时也会造成不必要的损失，' +
                        '请在执行操作前确认可以安全执行！</div>',
                    btn: ['重启工具箱进程', '结束工具箱进程', '取消进程操作'],
                    yes: function () {
                        layer.closeAll();
                        layer.confirm("这将会使 Tab Toolbox for Website 进行重载！", {title: '真的要重启吗？'},
                            function (index) {
                    ...
                    btn2: function () {
                        layer.closeAll();
                        layer.confirm("这将会使 Tab Toolbox for Website 进行不能使用！", {title: '真的要关闭吗？'},
                            function (index) {
                                layer.msg('等待服务器关闭......', {time: -1});
                                layui.use(['jquery'], function () {
                                    let $ = layui.jquery;
                                    $.ajax({
                                        url: "/api/process",
                                        ...
                                        dataType: "json",
                                        success: function () {
                                            layer.confirm("Tab Toolbox for Website 已经关闭，点击 [确定] 关闭页面！", {title: '关闭进程成功'},
                                                function (index) {
